...
def entryPoint(args):
  ...
  try:
    with open(pargs.invocation_info, 'r') as f:
      invocationInfo = InvocationInfo.loadInvocationInfo(f)
  except InvocationInfo.InvocationInfoValidationError as e:
    _logger.error('Failed to validate invocation info')
    _logger.error(e)
    _logger.debug(traceback.format_exc())
    return 1
